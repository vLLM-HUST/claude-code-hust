...
<p align="center">
  <img src="docs/images/app-icon.png" alt="Claude Code Hust" width="240">
</p>
...
[![Docs](https://img.shields.io/badge/📖_文档站点-Visit-FF7A00)](https://claudecode-hust.relakkesyang.org)

</div>

Claude Code Hust 基于 2026-03-31 从 Anthropic npm registry 泄露的 Claude Code 源码修复而来，现在主要是一个**桌面端 Claude Code 工作台**：把会话、多项目、分支 / Worktree、右侧代码改动、代码 Diff、权限审批、模型提供商、Computer Use、H5 远程访问、IM 接入和定时任务集中到一个 macOS / Windows / Linux APP 里。
...
Claude Code Hust 的桌面端把会话、多项目、分支 / Worktree、右侧代码改动、代码 Diff、权限确认、提供商配置和远程入口集中到一个图形化工作台里，适合不想长期停留在终端里的日常开发工作流。

<p align="center">
  <a href="https://github.com/NanmiCoder/cc-hust/releases"><img src="https://img.shields.io/badge/⬇_下载桌面端-macOS_%7C_Windows_%7C_Linux-FF7A00?style=for-the-badge" alt="下载桌面端"></a>
  &nbsp;
...
1. 前往 [Releases](https://github.com/NanmiCoder/cc-hust/releases) 下载 macOS / Windows / Linux 桌面端安装包。
...
- **vLLM** 已启动（默认端口 18000），例如：
  ```bash
  vllm-hust serve /data/shared-models/Qwen3-32B --served-model-name Qwen3-32B \
    --port 18000 --tensor-parallel-size 2 --max-model-len 32768 \
    --enable-auto-tool-choice --tool-call-parser hermes
  ```
- **LiteLLM**（脚本会自动启动，无需手动操作）：
  ```bash
  pip install 'litellm[proxy]'
  ```
- 配置 `.env`（已有默认配置，可按需修改模型名称）：
  ```bash
  cp .env.example .env
  # 修改 ANTHROPIC_MODEL 和 CLAUDE_CODE_MODEL_CONTEXT_WINDOWS 即可切换模型
  ```

### 一键启动 Web UI（推荐）

```bash
HOST=0.0.0.0 bash scripts/start-web-ui.sh
```

该脚本会自动完成以下全部步骤：
1. **Bootstrap** — 安装依赖、信任 postinstall 脚本、修复 bun 模块解析
2. **vLLM 检查** — 确认 vLLM 服务可达
3. **LiteLLM 代理** — 自动启动 Anthropic → OpenAI 协议转换代理（端口 4000）
4. **后端服务** — 启动 Claude Code Hust 后端（端口 3456）
5. **Web UI** — 启动 Vite 开发服务器（端口 2024）

启动后在浏览器访问 `http://localhost:2024` 即可使用。
...
同样自动完成 bootstrap、vLLM 检查、LiteLLM 代理启动，然后启动终端 CLI。

### 环境变量

| 变量 | 默认值 | 说明 |
|---|---|---|
| `ANTHROPIC_MODEL` | `Qwen3-32B` | 模型名称 |
| `ANTHROPIC_BASE_URL` | `http://localhost:4000` | LiteLLM 代理地址 |
| `CLAUDE_CODE_MODEL_CONTEXT_WINDOWS` | `{"Qwen3-32B":32768}` | 模型上下文窗口 |
| `CLAUDE_CODE_MAX_OUTPUT_TOKENS` | `8000` | 最大输出 token 数 |
| `VLLM_PORT` | `18000` | vLLM 服务端口 |
| `LITELLM_PORT` | `4000` | LiteLLM 代理端口 |
| `HOST` | `127.0.0.1` | 绑定地址（Web UI 用 `0.0.0.0` 可局域网访问） |
| `SKIP_BOOTSTRAP` | `0` | 设为 `1` 跳过依赖安装 |
| `SKIP_LITELLM` | `0` | 设为 `1` 跳过 LiteLLM 启动 |

切换模型只需修改 `.env` 中的 `ANTHROPIC_MODEL` 和 `CLAUDE_CODE_MODEL_CONTEXT_WINDOWS`。
...
| [全局使用](docs/guide/global-usage.md) | 在任意目录启动 claude-hust |
...
如果这个项目对您有帮助，请给个 ⭐ Star 支持一下，让更多的人看到 Claude Code Hust！
