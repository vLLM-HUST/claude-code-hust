...
export class DiagnosticsService {
  ...
  getLogDir(): string {
    return path.join(this.getConfigDir(), 'cc-hust', 'diagnostics')
  }
  ...
  async recordEvent(input: DiagnosticEventInput): Promise<void> {
    ...
    // ~/.claude/cc-hust/diagnostics. Tests that genuinely exercise diagnostics
  ...
  async exportBundle(): Promise<DiagnosticsExportResult> {
    ...
    const fileName = `cc-hust-diagnostics-${timestamp}.tar.gz`
  ...
  private async buildProvidersSummary(): Promise<Record<string, unknown>> {
    const providerPath = path.join(this.getConfigDir(), 'cc-hust', 'providers.json')
    try {
