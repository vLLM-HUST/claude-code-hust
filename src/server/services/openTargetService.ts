...
async function defaultConvertIconToPng(iconPath: string, size: number): Promise<Uint8Array> {
  ...
  const tmpDir = await mkdtemp(join(tmpdir(), 'cc-hust-open-target-icon-'))
...
$source = $env:CC_HUST_ICON_SOURCE
$output = $env:CC_HUST_ICON_OUTPUT
...
`
  ...
  await execFile('powershell.exe', [
    ...
    env: {
      ...process.env,
      CC_HUST_ICON_SOURCE: iconPath,
      CC_HUST_ICON_OUTPUT: outputPath,
    },
    timeout: 5_000,
    windowsHide: true,
  })
