/**
 * Integration tests for /api/hust-oauth/* endpoints.
 */
...
import { handleHustOAuthApi } from '../api/hust-oauth.js'
...
async function setup() {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'hust-oauth-api-test-'))
...
describe('POST /api/hust-oauth/start', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns authorize URL with PKCE challenge', async () => {
    const { req, url, segments } = buildReq('POST', '/api/hust-oauth/start', {
      serverPort: 54321,
    })
    const res = await handleHustOAuthApi(req, url, segments)
...
describe('GET /api/hust-oauth/status', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns loggedIn=false when no token file', async () => {
    const { req, url, segments } = buildReq('GET', '/api/hust-oauth/status')
    const res = await handleHustOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
  ...
  test('returns loggedIn=false when stored token is expired and refresh fails', async () => {
    ...
    hustOAuthService.setRefreshFn(async () => {
...
describe('DELETE /api/hust-oauth', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('clears token file', async () => {
    ...
    const { req, url, segments } = buildReq('DELETE', '/api/hust-oauth')
    const res = await handleHustOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
    expect(await hustOAuthService.loadTokens()).toBeNull()
  })
})
