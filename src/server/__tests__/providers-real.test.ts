 ...
 * 验证添加、激活、cc-hust/settings.json 同步是否正确
 * (provider env 写到 ~/.claude/cc-hust/settings.json，不污染原版 settings.json)
 */
...
describe('Real Provider Configs', () => {
  ...
  // Helper: read the Hust-specific settings file
  async function readCcHustSettings(): Promise<Record<string, unknown>> {
    const raw = await fs.readFile(path.join(tmpDir, 'cc-hust', 'settings.json'), 'utf-8')
  ...
  test('添加 MiniMax Provider 并激活 — 写入 cc-hust/settings.json', async () => {
    ...
    // 验证写入 cc-hust/settings.json
    const settings = await readCcHustSettings()
    ...
    console.log('✅ Provider 写入 cc-hust/settings.json，原版 settings.json 未被污染')
  })

  test('切换 Provider — 更新 cc-hust/settings.json', async () => {
    const minimax = await service.addProvider({
      presetId: 'minimax',
      name: 'MiniMax',
      baseUrl: 'https://api.minimaxi.com/anthropic',
    ...
    // 先激活 MiniMax
    await service.activateProvider(minimax.id)
    let settings = await readCcHustSettings()
    expect((settings.env as Record<string, string>).ANTHROPIC_BASE_URL).toBe('https://api.minimaxi.com/anthropic')
    expect(JSON.parse((settings.env as Record<string, string>).CLAUDE_CODE_MODEL_CONTEXT_WINDOWS)).toMatchObject({
      'MiniMax-M3': 1000000,
      'MiniMax-M2.7': 204800,
    ...
    settings = await readCcHustSettings()
    ...
    console.log('✅ 切换 Provider 成功，cc-hust/settings.json 更新正确')
  })

  test('cc-hust/settings.json 保留已有字段', async () => {
    // 预写一个有内容的 cc-hust/settings.json（模拟用户已有配置）
    ...
    console.log('✅ cc-hust/settings.json 已有字段全部保留')
  })
  ...
  test('providers.json 和 cc-hust/settings.json 独立于 settings.json', async () => {
    ...
    // 验证 cc-hust/settings.json 是 Hust 自己的
    const hust = await readCcHustSettings()
    expect((hust.env as Record<string, string>).ANTHROPIC_BASE_URL).toBe('https://api.minimaxi.com/anthropic')
    expect((hust.env as Record<string, string>).ANTHROPIC_AUTH_TOKEN).toBe('sk-hust-key')
    expect((hust.env as Record<string, string>).ANTHROPIC_API_KEY).toBe('')

    console.log('✅ 原版 settings.json 完好无损，Hust 配置独立存储')
  })
})
