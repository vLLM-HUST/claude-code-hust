/**
 * Hust OpenAI OAuth REST API
 *
 * POST   /api/hust-openai-oauth/start    — 生成 PKCE+state,返回 authorize URL
 * GET    /auth/callback                  — 用户浏览器 redirect 到此,完成 token 交换
 * GET    /callback/openai                — 兼容旧路径
 * GET    /api/hust-openai-oauth          — 查询当前登录状态(不回传 token 本体)
 * DELETE /api/hust-openai-oauth          — 登出,删除 token 文件
 */
...
export async function handleHustOpenAIOAuthApi(
  req: Request,
  url: URL,
  segments: string[],
): Promise<Response> {
  try {
    const action = segments[2] // segments: ['api', 'hust-openai-oauth', <action?>]
    ...
    if (action === 'start' && req.method === 'POST') {
      ...
      const session = await hustOpenAIOAuthService.startSession({
    ...
    if (action === undefined && req.method === 'GET') {
      const tokens = await hustOpenAIOAuthService.ensureFreshTokens()
      if (!tokens) {
    ...
    if (action === undefined && req.method === 'DELETE') {
      await hustOpenAIOAuthService.deleteTokens()
...
export async function handleHustOpenAIOAuthCallback(url: URL): Promise<Response> {
  ...
  try {
    await hustOpenAIOAuthService.completeSession(code, state)
