...
SESSION_NAME="cc-hust-context-live-${RUN_ID}"
ARTIFACT_DIR="${ARTIFACT_DIR:-$(mktemp -d "/tmp/cc-hust-context-live-${RUN_ID}-XXXX")}"
...
"${AB[@]}" eval "localStorage.setItem('cc-hust-locale', 'en'); localStorage.setItem('cc-hust-open-tabs', '${TAB_STATE}'); location.reload();" >>"${BROWSER_LOG}" 2>&1
