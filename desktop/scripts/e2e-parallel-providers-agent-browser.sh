...
ARTIFACT_DIR="${ARTIFACT_DIR:-/tmp/cc-hust-e2e-parallel-providers-$$}"
CONFIG_DIR="${ARTIFACT_DIR}/config"
PROJECT_A="${ARTIFACT_DIR}/project-a"
PROJECT_B="${ARTIFACT_DIR}/project-b"
SESSION_NAME_A="${SESSION_NAME_A:-cc-hust-provider-a-$$}"
SESSION_NAME_B="${SESSION_NAME_B:-cc-hust-provider-b-$$}"
...
mkdir -p "${ARTIFACT_DIR}" "${CONFIG_DIR}/cc-hust" "${PROJECT_A}" "${PROJECT_B}"
...
mkdirSync(join(configDir, 'cc-hust'), { recursive: true })
writeFileSync(join(configDir, 'cc-hust', 'providers.json'), JSON.stringify({
  schemaVersion: 1,
...
setup_browser() {
  ...
  "${ab[@]}" eval "
    ...
    localStorage.setItem('cc-hust-session-runtime', JSON.stringify({
