...
describe('doctorRepair', () => {
  ...
  it('clears only the safe desktop UI storage keys', () => {
    ...
    window.localStorage.setItem('cc-hust-chat-history', 'preserve')
    window.localStorage.setItem('cc-hust-provider-config', 'preserve')
    ...
    expect(window.localStorage.getItem('cc-hust-chat-history')).toBe('preserve')
    expect(window.localStorage.getItem('cc-hust-provider-config')).toBe('preserve')
  })
  ...
  it('keeps local repair successful when the server doctor endpoint is unavailable', async () => {
    ...
    expect(result.local.removedKeys).toContain('cc-hust-theme')
