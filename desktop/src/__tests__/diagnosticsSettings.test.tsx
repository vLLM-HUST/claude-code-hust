...
describe('Settings > Diagnostics tab', () => {
  beforeEach(() => {
    ...
    diagnosticsApiMock.getStatus.mockResolvedValue({
      logDir: '/tmp/claude/cc-hust/diagnostics',
      diagnosticsPath: '/tmp/claude/cc-hust/diagnostics/diagnostics.jsonl',
      cliDiagnosticsPath: '/tmp/claude/cc-hust/diagnostics/cli-diagnostics.jsonl',
      runtimeErrorsPath: '/tmp/claude/cc-hust/diagnostics/runtime-errors.log',
      exportDir: '/tmp/claude/cc-hust/diagnostics/exports',
      retentionDays: 7,
    ...
    diagnosticsApiMock.exportBundle.mockResolvedValue({
      bundle: {
        path: '/tmp/claude/cc-hust/diagnostics/exports/cc-hust-diagnostics.tar.gz',
        fileName: 'cc-hust-diagnostics.tar.gz',
        bytes: 1024,
      },
    })
    ...
    doctorRepairMock.runDoctorRepair.mockResolvedValue({
      local: {
        removedKeys: ['cc-hust-open-tabs', 'cc-hust-session-runtime'],
        missingKeys: ['cc-hust-theme', 'cc-hust-locale', 'cc-hust.persistence.schemaVersion'],
        failedKeys: [],
      },
      server: {
        ok: true,
  ...
  it('shows diagnostics status, actions, and recent events', async () => {
    ...
    expect(screen.getByText('/tmp/claude/cc-hust/diagnostics')).toBeInTheDocument()
  ...
  it('exports a diagnostics bundle from the settings page', async () => {
    ...
    expect(await screen.findByText('/tmp/claude/cc-hust/diagnostics/exports/cc-hust-diagnostics.tar.gz')).toBeInTheDocument()
  })
  ...
  it('runs Doctor from Diagnostics without clearing unrelated desktop state', async () => {
    window.localStorage.setItem('cc-hust-open-tabs', '{"activeTabId":"__settings__"}')
    window.localStorage.setItem('cc-hust-theme', 'dark')
    window.localStorage.setItem('cc-hust-chat-history', 'keep')
    ...
    expect(window.localStorage.getItem('cc-hust-chat-history')).toBe('keep')
  })
})
