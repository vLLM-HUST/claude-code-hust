...
export const jp: Record<TranslationKey, string> = {
  ...
  'settings.terminal.description': 'プラグイン、スキル、MCP のセットアップ用にホストマシンのコマンドを実行します。デスクトップアプリには claude-hust が含まれています。ドキュメントの claude <args> を claude-hust <args> に置き換えてください。例: claude-hust plugin install ... や claude-hust mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': 'クリアされるのは次のみ: cc-hust-open-tabs、cc-hust-session-runtime、cc-hust-theme、cc-hust-locale、cc-hust.persistence.schemaVersion。',
  ...
  'settings.providers.settingsJsonDesc': '~/.claude/cc-hust/settings.json — 直接編集できます。保存時に書き込まれます。',
  ...
  'settings.general.storagePortableDirPlaceholder': 'cc-hust データ用のフォルダを選択',
  ...
  'settings.general.storageMoveHint': 'ディレクトリを切り替えても既存のデータは移行されません。以前のセッションを引き続き表示したい場合は、古いディレクトリから projects、skills、plugins、cc-hust および関連フォルダをコピーしてください。ポータブルバンドルにする場合は、このフォルダをアプリの隣に置いて一緒に zip 化してください。',
  ...
  'settings.general.notificationsTestTitle': 'Claude Code Hust の通知が有効になりました',
  ...
  'settings.general.traceHintOn': '新しいセッションは、簡潔な要求、応答、状態イベントをローカルの cc-hust traces ディレクトリに書き込みます。',
