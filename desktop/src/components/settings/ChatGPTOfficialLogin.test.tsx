...
const initialOAuthState = useHustOpenAIOAuthStore.getState()
...
describe('ChatGPTOfficialLogin', () => {
  ...
  afterEach(() => {
    act(() => {
      ...
      useHustOpenAIOAuthStore.setState(initialOAuthState)
    })
    vi.useRealTimers()
    cleanup()
    vi.restoreAllMocks()
  ...
  it('keeps an actionable authorization link when shell open fails', async () => {
    ...
    expect(useHustOpenAIOAuthStore.getState().error).toBeNull()
