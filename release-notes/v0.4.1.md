...
- 如果本版本仍未完成 Apple / Windows 代码签名，首次安装时系统可能需要手动放行一次（见下方安装说明）。覆盖安装不会删除本地数据（`~/.claude`、`~/.claude/cc-hust` 及自定义便携数据目录）。
...
| 平台 | 文件 |
|------|------|
| macOS Apple Silicon（M 系列） | `Claude-Code-Hust-0.4.1-mac-arm64.dmg` |
| macOS Intel | `Claude-Code-Hust-0.4.1-mac-x64.dmg` |
| Windows x64 | `Claude-Code-Hust-0.4.1-win-x64.exe` |
| Linux x64 | `Claude-Code-Hust-0.4.1-linux-x86_64.AppImage` 或 `Claude-Code-Hust-0.4.1-linux-amd64.deb` |
| Linux ARM64 | `Claude-Code-Hust-0.4.1-linux-arm64.AppImage` 或 `Claude-Code-Hust-0.4.1-linux-arm64.deb` |
...
```bash
chmod +x Claude-Code-Hust-0.4.1-linux-x86_64.AppImage
./Claude-Code-Hust-0.4.1-linux-x86_64.AppImage
```

deb：

```bash
sudo apt install ./Claude-Code-Hust-0.4.1-linux-amd64.deb
```
