...
describe('Feishu: project picker card', () => {
  const sampleProjects: RecentProject[] = [
    {
      projectPath: '/Users/dev/claude-code-hust',
      realPath: '/Users/dev/claude-code-hust',
      projectName: 'claude-code-hust',
      isGit: true,
      repoName: 'claude-code-hust',
      branch: 'main',
  ...
  it('info column has title markdown + notation path markdown', () => {
    ...
    expect(info[0].content).toContain('**claude-code-hust**')
    ...
    expect(info[1].content).toContain('claude-code-hust')
  })
  ...
  it('row button says 选择 with small size and carries per-project value', () => {
    ...
    expect(btn1.value.realPath).toBe('/Users/dev/claude-code-hust')
    expect(btn1.value.projectName).toBe('claude-code-hust')
...
describe('Feishu: card.action.trigger parsing', () => {
  ...
  it('parses pick_project action from event', () => {
    ...
    expect(event.action.value.realPath).toBe('/Users/dev/claude-code-hust')
    expect(event.action.value.projectName).toBe('claude-code-hust')
  })
