...
```bash
./bin/claude-hust                          # Interactive TUI mode
./bin/claude-hust -p "your prompt here"    # Headless mode
./bin/claude-hust --help                   # Show all options
```
...
```bash
CLAUDE_CODE_FORCE_RECOVERY_CLI=1 ./bin/claude-hust
```
