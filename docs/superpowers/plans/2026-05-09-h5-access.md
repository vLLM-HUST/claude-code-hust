...
describe('H5AccessService', () => {
  ...
  it('preserves unrelated managed settings fields', async () => {
    ...
    const saved = JSON.parse(await fs.readFile(path.join(tmpDir, 'cc-hust', 'settings.json'), 'utf-8'))
...
- Persist under `cc-hust/settings.json` in an `h5Access` object.
...
```ts
const H5_SERVER_URL_KEY = 'cc-hust-h5-server-url'
const H5_TOKEN_KEY = 'cc-hust-h5-token'
```
...
- Ambiguity resolved: H5 config is stored in `~/.claude/cc-hust/settings.json`, not the shared `~/.claude/settings.json`.
